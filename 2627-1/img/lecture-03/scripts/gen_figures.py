...
plt.rcParams.update({"font.size": 14, "figure.facecolor": "white", "savefig.facecolor": "white"})

BLUE, ORANGE, GREEN, GRAY = "#1E93AB", "#E8890C", "#2E8B57", "#8a8a8a"
...
ax.bar_label(bars, [f" {t_np*1000:.0f} ms", f" {t_list*1000:.0f} ms"], fontsize=14, fontweight="bold")
ax.set_xlabel("thời gian nhân 5 triệu số với 1.1 rồi cộng tổng (giây)")
ax.set_xlim(0, t_list * 1.28)
ax.spines[["top", "right"]].set_visible(False)
ax.set_title(f"Cùng một việc — NumPy nhanh hơn ~{t_list / t_np:.0f} lần", fontweight="bold")
fig.tight_layout()
fig.savefig(OUT / "speed-comparison.png", dpi=150)
plt.close(fig)
...
def draw_grid(x0, y0, rows, cols, vals, color, alpha=1.0, dashed=False):
    for r in range(rows):
        for c in range(cols):
            rect = plt.Rectangle((x0 + c * CELL, y0 + (rows - 1 - r) * CELL), CELL, CELL,
                                 facecolor=color, edgecolor="#333", alpha=alpha,
...
draw_grid(0.3, 0.5, 4, 3, A, "#dbeef3")
...
B = [[1, 2, 3]]
draw_grid(4.5, 0.5 + 3 * CELL, 1, 3, B, "#fdf0dd")
for k in range(3):
    draw_grid(4.5, 0.5 + k * CELL, 1, 3, B, "#fdf0dd", alpha=0.35, dashed=True)
...
draw_grid(9.8, 0.5, 4, 3, Cm, "#e3f2e9")
ax.text(9.8 + 1.5 * CELL, 4.35, "kết quả (4, 3)", ha="center", fontsize=13, color="#20623f")

fig.tight_layout()
fig.savefig(OUT / "broadcasting.png", dpi=150)
plt.close(fig)
print("broadcasting.png done")
...
for ax, (title, color) in zip(axes, [("axis=0 — dọc theo hàng\n→ ra 1 số MỖI CỘT", BLUE),
    ...
    for r in range(4):
        for c in range(3):
            ax.add_patch(plt.Rectangle((c, 3 - r), 0.92, 0.92, facecolor="#f2f2f2", edgecolor="#555"))
            ax.text(c + 0.46, 3 - r + 0.46, M[r][c], ha="center", va="center", fontsize=12)
    if "axis=0" in title:
        for c in range(3):
...
fig.suptitle("mean(axis=?) trên ma trận (4 thành phố × 3 tháng)", fontsize=14, fontweight="bold")
fig.tight_layout()
fig.savefig(OUT / "axis.png", dpi=150)
plt.close(fig)
print("axis.png done")
